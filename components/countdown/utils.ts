...
export function calcaulateDiff(timeInMs: number) {
...
function getRemainingSeconds(nowDayjs:dayjs.Dayjs, timestamDayjs: dayjs.Dayjs) {
  const seconds = timestamDayjs.diff(nowDayjs, "seconds") % 60;
  return padWithZeros(seconds, 2);
}
function getRemainingMinutes(nowDayjs: dayjs.Dayjs, timestamDayjs: dayjs.Dayjs) {
  const minutes = timestamDayjs.diff(nowDayjs, "minutes") % 60;
  return padWithZeros(minutes, 2);
}
function getRemainingHours(nowDayjs: dayjs.Dayjs, timestamDayjs: dayjs.Dayjs) {
  const hours = timestamDayjs.diff(nowDayjs, "hours") % 60;
  return padWithZeros(hours, 2);
}
function getRemainingDays(nowDayjs: dayjs.Dayjs, timestamDayjs: dayjs.Dayjs) {
