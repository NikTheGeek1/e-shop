...
export default function Images({ images, setImages }: ImagesProps) {
  ...
  const handleImages = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach((img, i) => {
      ...
      if (
        ...
        reader.onload = (e) => {
          setImages((images: File[]) => [...images, e.target?.result]);
        };
      }
    });
  };
  const removeImage = (image: File) => {
    setImages((images: File[]) => images.filter((img) => img !== image));
    if (images.length <= 3) {
      setError("");
    }
  };
